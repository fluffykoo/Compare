...
public class CompareJsonFiles {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 4) {
            System.out.println("Usage: java CompareJsonFiles <refFile> <newFile> <outputFolder> <configFile>");
            return;
        }
        ...
        ReportGenerator gen = new ReportGenerator(outFolder, ts, "JSON_Report");
        ...
        for (String id : refMap.keySet()) {
            if (!newMap.containsKey(id)) deleted++;
                ...
                boolean isModified = diffs.stream()
                        .anyMatch(d -> d.getEntityId().equals(id) && d.getType() == ChangeType.MODIFICATION);
